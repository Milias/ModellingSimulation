...
On the other hand, for the right peak we don't have this limitation and thus the shape is closer to a power law as we were expecting. Even in this plot we are not represeting any error bars, the method used to compute this values (by hand) introduces quite large amounts of error, so we can expect not to find a particularly good approximation of the functional dependece for the right peak. This can be improved by using a high density initial configuration and focus in the right peak, instead of starting from a low density configuration for both.
