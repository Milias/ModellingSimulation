...
def HeatVsT(start, savefile, filenames):
  ...
  M_l = {}
  for i in range(len(filenames)):
    E = array(data[i]["Energy"])[start:]/data[i]["TotalSize"]
    if not data[i]["Size"] in M_l:
      M_l[data[i]["Size"]] = [[],[],[],[]]
  ...
  if dim == 2:
    ...
    alpha = linspace(0.11008,1.77,1)
    nu = linspace(0.629971,1.01,1)
    ...
    for j in nu:
      for k in T_c:
        ...
        for m in M_l:
          t = (M_l[m][0]-k)/k
          #plt.plot(M_l[m][0], M_l[m][1], "-", label="L = %d" % m, linewidth=1.2)
          plt.errorbar(t*m**(1/j), M_l[m][1]*m**(i/j), yerr=M_l[m][3]*m**(i/j), fmt="-", label="L = %d" % m, linewidth=1.2)
          plt.xlim([-10,20])
        plt.title("Magnetic susceptibility vs Temperature\n"+r"$\gamma = %1.3f, \nu = %1.3f, T_c = %1.3f$" % (i, j, k))
        plt.legend(loc=0, numpoints=1)
        plt.savefig("report/graphs/fit-spec-%d/fit-%d.png" % (dim,nf))
        plt.show()
        nf += 1
  #"""
  return "Success."
...
def SuscVsT(start, savefile, filenames):
  ...
  if dim == 2:
    ...
    T_c = linspace(4.51,4.52,1)
    gamma = linspace(1.237075,1.77,1)
    nu = linspace(0.629971,1.01,1)
